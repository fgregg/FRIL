...
public class FixedWidthConfigurationPanel extends GUIVisibleComponent {
	
	private ParamsPanel panel;
	
	public Object generateSystemComponent() throws RJException, IOException {
		AbstractDataSource source = new FixedColumnWidthFileDataSource(
	...
	public JPanel getConfigurationPanel(Object[] params, int sizeX, int sizeY) {
		...
		panel = new ParamsPanel(
				availableparams, 
				new String[] {"Source name", "File name", "Data model file name"}, 
				defaults,
				creators);
		
		Map validators = new HashMap();
		validators.put(AbstractDataSource.PARAM_SOURCE_NAME, new NonEmptyValidator());
		validators.put(FixedColumnWidthFileDataSource.PARAM_FILE_NAME, new NonEmptyValidator());
	...
	public String toString() {
		return "Fixed width columns input file";
	}

	public Class getProducedComponentClass() {
		return FixedColumnWidthFileDataSource.class;
	}
