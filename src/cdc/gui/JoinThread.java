	...
	public void run() {
		...
		try {
			...
			for (int i = 0; i < system.getResultSavers().length; i++) {
				system.getResultSavers()[i].reset();
			}
			...
			while ((row = system.getJoin().joinNext()) != null) {
				n++;
				for (int i = 0; i < system.getResultSavers().length; i++) {
					system.getResultSavers()[i].saveRow(row);
					SwingUtilities.invokeAndWait(new Runnable() {
						public void run() {
			...
			for (int i = 0; i < system.getResultSavers().length; i++) {
				system.getResultSavers()[i].close();
			}
